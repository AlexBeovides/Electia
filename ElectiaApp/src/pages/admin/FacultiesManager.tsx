...
export const FacultiesManager = () => {
  ...
  const colDefs: ColDef<IRow>[] = useMemo(() => [
    // { field: 'id', headerName: 'ID', editable: false }, // ID should not be editable
  ...
  const onCellValueChanged = (params: any) => {
    ...
    fetch(`${API_BASE_URL}/Faculties/${updatedData.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
  ...
  const deleteSelectedRows = () => {
    ...
    Promise.all(
      selectedIds.map(id =>
        fetch(`${API_BASE_URL}/Faculties/${id}`, {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setNewFaculty({ ...newFaculty, [event.target.name]: event.target.value });
  };

  return (
    <>
      <div className="manager-page-container"> 
        <h2 className="page-header">Facultades:</h2>
